...
func CalcStoreTotal(data ProductData) {
	...
	var channel chan float64 = make(chan float64, 100) // Create channel
	for category, group := range data {
		go group.TotalPrice(category, channel) // Passing channel to a function
	}
	// for i := 0; i < len(data); i++ {
	// 	storeTotal += <-channel // Adding result from channel
	// }
...
func (group ProductGroup) TotalPrice(category string, resultChannel chan float64) {
	...
	resultChannel <- total // Writing to the channel
